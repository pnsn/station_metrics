...
#listlens = [ len(label4s[i]) for i in range(0,len(label4s))  ]
#np.argmax(np.cumsum(listlens) > 9)
        if ( len(label5) < 18 ):
...
def make_station_figure_wthresholds(st1,st2,st3,st4,st5,label1,label2,label3,label4,label5,thresh1,thresh2,thresh3,thresh4,thresh5):
        ...
        for ii in range(0,len(st3)):
            ...
            if ( thresh3 > 0 ):
                ...
                ax3.plot([t1,t2],[-1*thresh3,-1*thresh3],color='r',linewidth=0.5)
            ax3.xaxis.set_major_formatter(DateFormatter('%H:%M'))
        for ii in range(0,len(st4)):
            timearray = np.arange(st4[ii].stats.npts)*st4[ii].stats.delta
            t = st4[ii].stats.starttime
...
def make_station_figure_talk(st1,st2,st3,st4,st5,label1,label2,label3,label4,label5,thresh1,thresh2,thresh3,thresh4,thresh5):
        ...
        for ii in range(0,len(st4)):
            ...
            t = st4[ii].stats.starttime
            x4 = np.array([datetime.datetime(t.year,t.month,t.day,t.hour,t.minute,t.second,t.microsecond) + datetime.timedelta(seconds=jj) for jj in timearray ])
            ax2.plot(x4,st4[ii].data,color='k',linewidth=0.2)
            if ( thresh2 > 0 ):
                ax2.plot([t1,t2],[thresh2,thresh2],color='r',linewidth=0.5)
                ax2.plot([t1,t2],[-1*thresh2,-1*thresh2],color='r',linewidth=0.5)
            ax2.xaxis.set_major_formatter(DateFormatter('%H:%M'))
...
#            ax3.xaxis.set_major_formatter(DateFormatter('%H:%M'))
            ...
            iRMSwinlen = int(5.0/st4[ii].stats.delta)
            rmsthing = np.sqrt(smooth(((st4[ii].data)**2),iRMSwinlen))

            ax3.plot(x3,st4[ii].data,color='k',linewidth=0.2)
            ax3.plot(x3,rmsthing,color='g',linewidth=1)
            if ( thresh3 > 0 ):
                ax3.plot([t1,t2],[thresh3,thresh3],color='r',linewidth=0.5)
#                ax3.plot([t1,t2],[-1*thresh3,-1*thresh3],color='r',linewidth=0.5)
            ax3.xaxis.set_major_formatter(DateFormatter('%H:%M'))
#            ax3.plot([t1,t2],[0,0],color='r',linewidth=0.5)
        ...
        for ii in range(0,len(st4)):
            ...
            if ( thresh4 > 0 ):
                ...
                ax4.plot([t1,t2],[-1*thresh4,-1*thresh4],color='r',linewidth=0.5)
            ax4.xaxis.set_major_formatter(DateFormatter('%H:%M'))
#            ax4.plot([t1,t2],[0,0],color='r',linewidth=0.5)
