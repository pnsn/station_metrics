...
Approximate ElarmS3/EPIC Trigger Count, Highpassed
...
displacement highpassed at 0.075 Hz, and by a boxcar test that rejects DC
steps. The implementation follows A. I. Chung et al., SRL March/April 2019.

## Uses

This is the closest thing available to a direct estimate of a station's
contribution to EPIC's trigger load, calculated from archived data without
needing access to the running system. A station producing large numbers every
hour is feeding EPIC noise that has to be associated and rejected, which costs
processing and can degrade solutions. It is also useful before a station is
brought into ShakeAlert, to estimate what it would contribute. It is
deliberately an approximation: it uses a single channel's archived data and
knows nothing about the state of the real EPIC system, so it should be read as
a station health indicator rather than a prediction of EPIC's actual output.
...
2. Build three amplitude traces from the same raw data. For each of
   ...
   apply a causal Butterworth highpass at 0.075 Hz with 2 corners, cut to the
   analysis window, and remove the mean again. Call these a (m/s^2), v (m/s)
   and d (m).
3. Build the trigger trace independently: same preparation, converted to
   velocity and highpassed at 3 Hz rather than 0.075 Hz.
...
7. Apply the boxcar test. In a 0.1 s window beginning 0.1 s after the
   ...
   2.2e-8 m/s for a broadband. A digitizer or telemetry glitch is a DC step: it
   trips the STA/LTA but carries no oscillation behind it, so it fails here.
...
The amplitude gate values are expressed as log10 amplitudes in the original
EPIC formulation; the SI values quoted above are their decimal equivalents.
...
Only vertical component channels are analyzed, matching EPIC's own use of
vertical data for triggering.

## Change Log

Aug 2026: the boxcar test was previously applied to rectified data, measuring
max(|x|) - min(|x|). Rectifying collapses a symmetric oscillation toward zero
range, so the old test rejected more candidates than intended. It now measures
the signed range max(x) - min(x). Counts from before and after this change are
not directly comparable and should be expected to rise.

Aug 2026: the processing order changed. Previously the trace was cut to the
analysis window before integration, differentiation and filtering, so the
filter start-up transient and the integration drift fell inside the measured
window.
...
the amplitude gates use 0.075 Hz.
...
approximate_epic_bp_triggers, acc_spikes_gt_.34
