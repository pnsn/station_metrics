...
Noise Floor of Bandpassed Acceleration
...
been sensitivity corrected, converted to acceleration and bandpassed between
0.075 Hz and 15 Hz. It is computed as half the range between the 2nd and 98th
percentile of the samples, and reported in cm/s^2.

## Uses

Restricting the measurement to 0.075 - 15 Hz makes this the better indicator of
noise in the band that earthquake early warning actually uses. Trimming the
distribution at the 2nd and 98th percentile keeps earthquakes, calibration
pulses and one-off spikes from dominating a number meant to describe the quiet
background. A gradual rise over days or weeks usually means a failing sensor, a
loose connection, or a change in the site environment; a sudden step usually
means a hardware or configuration change.
...
10. Report half their difference, converted to cm/s^2:
   ...
   hourly_noise_floor_bp_acc = 100 * (a98 - a2) / 2

## Metric Values Returned

value - approximate noise floor of the bandpassed acceleration (cm/s^2).
...
98th is positive, so half their difference is a sensible amplitude scale.
...
hourly_noise_floor_acc, hourly_max_bp_acc, power_5Hz
